...
当n = 2的x次幂时,满足转换条件,**(n - 1) & hash 等价于 hash % n**, 为什么?

其实很简单: 因为的容量始终为 n = 2^x,所以我们使用二进制的形式来看(n - 1) & hash
* (n - 1)换成二进制: 2^x - 1 = 0000 111...11 (一共x个1,这里我们用...省略), 高位为0
* 再进行 & 运算: hash & 0000 111...11 , 再将hash二进制化, 因为高位计算都为0, 那么就相当于只有hash值的后x位上参与了计算
* 那么我们再想一下**x位以前的值**肯定是可以整除 n 的, 而剩下的**x以后的值**就是比 n 小的数, 即余数

这就很容易理解了, 实际就是通过与运算来直接得到一个比n小的数,这个就是hash的余数.
所以满足了(n -1) & hash = hash % n
### HashMap中线程不安全问题
* 扩容导致的链表死循环问题:
