...
| 线程种类 | 主要工作 |
| ------- | -------- | 
| 虚拟机线程 | 这个线程等待 JVM 到达安全点操作出现。这些操作必须要在独立的线程里执行,因为当(VM thread) 堆修改无法进行时,线程都需要 JVM 位于安全点。这些操作的类型有:stop-the-world 垃圾回收、线程栈 dump、线程暂停、线程偏向锁(biased locking)解除。 |
